...
function [NN,trerr]=nn_train(NN,NP,X,Y,opt,varargin)
...
if bAddUnsErr
    ...
    for li=1:lAmt-2
        if ~isfield(NN{li},'rB')
            NN{li}.rB=zeros(1,NP.layerSize(li));
        end
        if ~isfield(NN{li},'momentumrB')
            NN{li}.momentumrB=zeros(1,NP.layerSize(li));
        end
    end
    NN{1}.f=NN{2}.f;
    NN{1}.df=NN{2}.df;
...
%%%
trerr=zeros(batchAmt*opt.epochAmt,1); vaerr=trerr;
ct=0;
